    ...
    def require(self, required_scopes: Iterable[str]) -> None:
        ...
        if missing:
            raise HTTPException(
                ...
                detail={"message": "Missing required MemoryMesh scope", "missing_scopes": missing},
            )
...
async def resolve_principal(
    ...
    if raw_key:
        ...
        if not doc or doc.get("status") != "active":
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or inactive MemoryMesh API key")
    ...
    if settings.auth_required:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="MemoryMesh API key required")

    role = request.headers.get("X-MemoryMesh-Role", "owner")
    return EnterprisePrincipal(
