...
class PostgresStore:
    """
    PostgreSQL-backed document store for MemoryMesh.

    The service stores each MemoryMesh logical collection as JSONB
    ...
    async def insert_one(self, collection: str, doc: Dict[str, Any]) -> str:
        ...
        if self._memory_records is not None:
            self._memory_records.setdefault(collection, {})[record_id] = record
            return record_id
        created_at = self._coerce_datetime(record.get('created_at')) or datetime.now(timezone.utc)
        async with self._acquire() as conn:
            await conn.execute(
                """
                INSERT INTO memorymesh_records(collection, id, data, created_at, updated_at)
    ...
    async def find_one(self, collection: str, doc_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._acquire() as conn:
            row = await conn.fetchrow(
                'SELECT data FROM memorymesh_records WHERE collection = $1 AND id = $2',
                collection,
                doc_id,
            )
    ...
    async def find_many(
        ...
        if self._memory_records is not None:
            ...
            for key, value in (query or {}).items():
                values = [doc for doc in values if str(doc.get(key)) == str(value)]
            values = self._sort_memory_docs(values, sort)
            return values[:limit]
        where, params = self._build_where(collection, query or {})
        order_by = self._build_order_by(sort)
        sql = f'SELECT data FROM memorymesh_records WHERE {where} {order_by} LIMIT ${len(params) + 1}'
        params.append(limit)
        async with self._acquire() as conn:
    ...
    async def count_documents(self, collection: str, query: Dict[str, Any] | None = None) -> int:
        ...
        where, params = self._build_where(collection, query or {})
        async with self._acquire() as conn:
            return int(await conn.fetchval(f'SELECT count(*) FROM memorymesh_records WHERE {where}', *params))
    ...
    async def delete_all(self) -> None:
        if self._memory_records is not None:
            ...
            return
        async with self._acquire() as conn:
            await conn.execute('DELETE FROM memorymesh_records')
    ...
    def _sort_memory_docs(self, docs: list[Dict[str, Any]], sort: Sequence[tuple] | None) -> list[Dict[str, Any]]:
        if not sort:
            return sorted(docs, key=lambda doc: str(doc.get('created_at') or ''), reverse=True)
        sorted_docs = docs
        for key, direction in reversed(sort):
            reverse = direction == DESCENDING or str(direction).lower() == 'desc'
            sorted_docs = sorted(sorted_docs, key=lambda doc: doc.get(str(key)) or '', reverse=reverse)
        return sorted_docs
