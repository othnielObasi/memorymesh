...
from app.services.truefoundry_gateway import GatewayResult


class GoogleGeminiGatewayService:
    """Google/Gemini reference gateway adapter.

    This adapter keeps MemoryMesh's runtime model provider-agnostic. It is a
    ...
    async def chat(self, **kwargs: Any) -> GatewayResult:
        ...
        # MemoryMesh run, checkpoint, recovery, or audit model.
        return GatewayResult(
            content=(
                "MemoryMesh Google reference gateway response: runtime state, checkpointing, "
