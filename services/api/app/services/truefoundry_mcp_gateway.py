...
class TrueFoundryMCPGatewayService:
    """Adapter for routing agent tool calls through Optional MCP Gateway.

    The hackathon product story is strongest when both model calls and tool calls
    are gateway-visible. This service gives MemoryMesh a single tool-call path:

        MemoryMesh Agent -> Optional MCP Gateway -> Tool/API
    ...
    def enabled(self) -> bool:
        return bool(self.settings.truefoundry_mcp_gateway_url and self.settings.truefoundry_mcp_api_key)

    def _endpoint(self, tool_name: str) -> str:
        base = (self.settings.truefoundry_mcp_gateway_url or "").rstrip("/")
        path = self.settings.truefoundry_mcp_tool_invoke_path.format(tool_name=tool_name).lstrip("/")
        return f"{base}/{path}"
    ...
    async def call_tool(
        ...
        *,
        ...
        headers = {
            "Authorization": f"Bearer {self.settings.truefoundry_mcp_api_key}",
            "Content-Type": "application/json",
            "X-MemoryMesh-Tool": tool_name,
        }
        body = {
            "tool": tool_name,
            "input": payload,
